  ...
  public static void main(String[] args) {
    ...
    Map<Integer, List<String>> collect2 = students.stream()
        .collect(Collectors.groupingBy(
            ...
            Collectors.mapping(
                Student::getName,  // 다운스트림 1 : 학생 이름
                Collectors.toList())  // 다운스트림 2 : 리스트로 수집
        ));
    ...
    Map<Integer, Long> collect3 = students.stream()
        .collect(Collectors.groupingBy(
            Student::getGrade,
            Collectors.counting()  // 다운스트림 : 학생 수 세기
        ));
    System.out.println("collect3 = " + collect3);

    // 4단계 : 학년별로 학생들의 평균 점수를 출력해라.
    Map<Integer, Double> collect4 = students.stream()
        .collect(Collectors.groupingBy(
            Student::getGrade,
            Collectors.averagingInt(Student::getScore)  // 다운스트림 : 평균 점수 계산
        ));
