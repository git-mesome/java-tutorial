...
public class PrimitiveStreamMain {
  ...
  public static void main(String[] args) {
    ...
    LongStream mappedLong = IntStream.range(1, 5)
        .mapToLong(i -> i * 10L);
    System.out.println("mappedLong = " + mappedLong);

    // int -> double 변환 매핑
    DoubleStream mappedDouble = IntStream.range(1, 5)
        .mapToDouble(i -> i * 10.0);
    System.out.println("mappedDouble = " + mappedDouble);

    // int -> 객체 변환 매핑
    Stream<String> mappedObj = IntStream.range(1, 5)
        .mapToObj(i -> "Number: " + i);
    System.out.println("mappedObj = " + mappedObj);
